      ...
      this.cropper.getCropperImage()
        .then((src) => {
          wx.getImageInfo({
            src: src,
            success:(res)=>{
              this.$store.state.board.goLinks.push(res.path.split('/')[0]==='static'?'/'+res.path:res.path)
              this.$store.state.board.boards.push({key:res.path.split('/')[0]==='static'?'/'+res.path:res.path,checked:true})
            }
          })
